...
1. **A partner specifies the configuration for a USDC vault**, which Concrete deploys via the `ConcreteFactory` contract. The vault is configured with two strategies: a Pendle yield strategy and a Curve stable pool strategy.
